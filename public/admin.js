...
async function fetchResults() {
  try {
    ...
    if (maxP > 0) {
      ...
      // ★「グラフは表示されません」をやめる（今回は maxP に依存しないグラフのため）
      maxInfo.textContent = `想定人数は未設定です（現在の投票数：${total}人）`;
    }
